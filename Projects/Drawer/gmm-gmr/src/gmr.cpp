...
/* init the GaussianMixture by splitting the dataset into 
  ...
  {
    int s = (int)(abs((DataSet(n,0)/(tmax+1))*nState));
    //std::cout << s << std::endl;
  ...
  std::cout << "Data, tmax " << nData << tmax << std::endl;
}
...
Matrix GaussianMixture::speedCompute(Matrix inData){


	int nRows = inData.RowSize();
	int nCols = inData.ColumnSize();
	int i,j;
	Matrix Outpout(nRows-1, nCols);
	...
	for (i = 0; i < nRows-1; i++){
		for (j = 1; j < nCols; j++){
			Outpout (i, j) = inData(i+1,j) - inData(i, j);
		}
	}
...
Matrix GaussianMixture::speedPosition(Matrix inData, Matrix speedData){


	int rows = inData.RowSize() - 1;
	int cols = inData.ColumnSize() + speedData.ColumnSize() - 1;

	Matrix speedPos(rows, cols);


	for (int j = 0; j < rows; j++){
		speedPos(j,0) = (float)j;
		speedPos(j,8) = speedData(j, 1);
	}


	for (int i = 0; i < rows; i++){
		for (int j = 1; j < inData.ColumnSize(); j++){
		speedPos(i,j) = inData(i,j);
		}
		for (int j = 1; j < speedData.ColumnSize() - 1; j++){
		speedPos(i,j + inData.ColumnSize()) = speedData(i,j+1);
		}
	}
...
Matrix GaussianMixture::getPosition(Matrix outData, Vector posInit){

	Matrix Outpout(250, 8);

	for (int i = 0; i < 250; i++)
		Outpout(i, 0) = i;

	for (int j = 1; j < 8; j++)
		Outpout(0,j) = posInit(j - 1);

	for (int i = 1; i < 250; i++){
		for (int j = 1; j < 8; j++){
			Outpout(i,j) = Outpout(i-1, j) + outData(i-1,j-1);
		}
	}

/*	for (int i = 0; i < 250; i++)
		std::cout << Outpout(i, 7) << std::endl;*/
